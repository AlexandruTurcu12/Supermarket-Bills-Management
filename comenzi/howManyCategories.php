...
$sql = "SELECT C.ComandaID AS Comanda, COUNT(DISTINCT P.CategorieID) AS Numar FROM Comenzi C JOIN ComenziProduse CP
ON C.ComandaID = CP.ComandaID JOIN Produse P ON P.ProdusID = CP.ProdusID GROUP BY C.ComandaID";
...
if ($result->num_rows > 0) {
  ...
  echo"<tr><th>ID Comanda</th><th>Numar</th></tr>";
  while($row = $result->fetch_assoc()) {
    echo "<tr><td>{$row['Comanda']}</td><td>{$row['Numar']}</td></tr>";
  }
}
else {
  echo '0 results';
