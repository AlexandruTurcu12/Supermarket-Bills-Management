...
$sql = "SELECT C.Nume AS Cat, P.Nume AS Produs FROM Categorii C JOIN Produse P ON C.CategorieID = P.CategorieID
 GROUP BY P.Nume ORDER BY C.Nume, P.Nume";
...
if ($result->num_rows > 0) {
  ...
  echo"<tr><th>Nume</th><th>Produs</th></tr>";
  while($row = $result->fetch_assoc()) {
    echo "<tr><td>{$row['Cat']}</td><td>{$row['Produs']}</td></tr>";
  }
}
else {
  echo '0 results';
