...
$sql = "SELECT `CategorieID`, `Nume`, `Pret` FROM `Produse`"; 
$result = $conn->query($sql);

if ($result->num_rows > 0) {
  echo "<table class=center border='2'>";
  echo"<tr><th>ID Categorie</th><th>Nume</th><th>Pret</th></tr>";
  while($row = $result->fetch_assoc()) {
    echo "<tr><td>{$row['CategorieID']}</td><td>{$row['Nume']}</td><td>{$row['Pret']}</td></tr>";
  }
}
else {
  echo '0 results';
